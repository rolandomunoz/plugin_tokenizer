...
@tg.duplicate_tier: src_tier, "syllable"
...
for interval to nIntervals
    ...
    if interval_text$ <> ""
        @tokenizer.string2syllable_cvc: interval_text$, vowels$, consonants$, ""
        strID_syll = selected("Strings")

        selectObject: strID_syll
        plusObject: tgID
        @tokenizer.Strings2TextGrid: dst_tier, tmid
        removeObject: strID_syll
    endif
endfor
